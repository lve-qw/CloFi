...
async function loadProducts(clear = true) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (productsContainer && currentPage === 1) {
                productsContainer.innerHTML = `<p class="error">${errorMessage}</p>`;
            }
            hasMore = false;
            return;
        }
        ...
        if (!products || products.length === 0) {
            if (currentPage === 1 && productsContainer) {
        ...
        if (productsContainer && currentPage === 1) {
            productsContainer.innerHTML = '<p>Ошибка соединения с сервером</p>';
        }
        hasMore = false;
    } finally {
        isLoading = false;
        ...
        icon.addEventListener('click', async function() {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    console.error('Ошибка сервера:', err.error);
                    return;
                }
