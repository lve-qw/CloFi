  ...
  products.forEach(product => {
    ...
    productCard.innerHTML = `
      ...
      <div class="card-body">
        ...
        ${product.availability !== undefined ? `<p class="availability ${product.availability ? 'in-stock' : 'out-of-stock'}">
    ...
    icon.addEventListener('click', async function() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          const err = await res.json().catch(() => ({ error: 'Ошибка при удалении из избранного' }));
          messageContainer.textContent = err.error || 'Ошибка при удалении из избранного';
          return;
        }

        const data = await res.json();
        if (data.status === 'лайк удалён') {
        ...
        console.error('Ошибка:', error);
        messageContainer.textContent = 'Ошибка при удалении из избранного';
      }
    });
  });
}
